    ...
    const testTools = {
      get_planData: tool({
        ...
        execute: async ({ key }) => {
          ...
          const result = key === "storySkeleton" ? "" : "无数据";
          console.log(`   返回: ${result.substring(0, 50)}`);
          return result;
        },
      }),
      get_novel_events: tool({
        ...
        execute: async ({ chapterIndexs }) => {
          ...
          const result = "| 第1章 黄皮子拦路 | 易灵风、石娃、黄大仙 | 易灵风遇黄皮子拦车避劫遭雷击穿越，结识石娃后于破庙受黄大仙元神传授功法并接下救仙重任 | 强 | 高 | 55秒 | 转折+悬疑+情感 |";
          console.log(`   返回: ${result.substring(0, 50)}...`);
          return result;
        },
      }),
